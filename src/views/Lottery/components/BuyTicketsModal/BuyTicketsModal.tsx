...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.tx8.address)
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserCake.div(priceTicketInTX8)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase

      // If the users' max TX8 balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInTX8.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInTX8, discountDivisor, getTicketCostAfterDiscount])
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient TX8 balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInTX8.times(new BigNumber(ticketsToBuy))) : '0.00'} TX8`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInTX8 && getFullDisplayBalance(priceTicketInTX8.times(ticketsToBuy || 0))} TX8
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
