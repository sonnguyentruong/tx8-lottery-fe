...
const RewardBrackets: React.FC<RewardMatchesProps> = ({ lotteryNodeData, isHistoricRound }) => {
  ...
  useEffect(() => {
    if (lotteryNodeData) {
      const { treasuryFee, amountCollectedInTX8, rewardsBreakdown, countWinnersPerBracket } = lotteryNodeData

      const feeAsPercentage = new BigNumber(treasuryFee).div(100)
      const cakeToBurn = feeAsPercentage.div(100).times(new BigNumber(amountCollectedInTX8))
      const amountLessTreasuryFee = new BigNumber(amountCollectedInTX8).minus(cakeToBurn)
      setState({
        isLoading: false,
        cakeToBurn,
